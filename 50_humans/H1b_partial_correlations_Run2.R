# Script: H1b_partial_correlations_Run2.R
# Tests H1b convergent validity with partial correlations
# Controls: gender, education, age, flagged for exclusions (Questionnaire long string)
#
...
RUN_LABEL  <- "Run2_Son46_Tnull_PrD_partial_exlusion"
...
# --- Load flagged dataset and apply exclusions ---
df_flagged <- read.table("Data_Complete_Run2_flagged.csv", header = TRUE,
                         sep = "|", stringsAsFactors = FALSE)

flagged_ids <- df_flagged$participant_id[df_flagged$flag_exclude == TRUE]
cat("Flagged participants:", length(flagged_ids), "\n")

df <- df[!df$participant_id %in% flagged_ids, ]
cat("Participants after exclusion:", nrow(df), "\n\n")
...
# --- Control variables ---
controls <- c("gender", "education", "age")
...
# --- Run partial correlation for one dimension ---
run_partial_h1b <- function(q_var, llm_var, label, data, z_vars) {
  ...
  # Zero-order Pearson (no controls)
...
# ============================================================
# H1b: PARTIAL CORRELATIONS — EACH CONTROL SEPARATELY + COMBINED
# ============================================================
cat("=== H1b: Convergent Validity — Partial Correlations ===\n\n")
...
control_sets <- list(
  ...
  "education" = "education",
  "age"       = "age",
  "combined"  = c("gender", "education", "age")
)
...
for (dim in dimensions) {
  for (ctrl_name in names(control_sets)) {
    ...
    result    <- run_partial_h1b(dim$q, dim$llm, dim$label, df, ctrl_vars)
    result$control_set <- ctrl_name
    results_list[[length(results_list) + 1]] <- result
  }
}

h1b_partial <- bind_rows(results_list)

print(h1b_partial)
save_table(h1b_partial, "H1b_partial_correlations_separate")